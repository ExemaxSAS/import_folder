...
class StockMoveLineInheritDispatch(models.Model):
    ...
    dispatch_from_lot = fields.Char(string='Despacho de Ventas', compute='_compute_dispatch_from_lot')
   
    def _compute_dispatch_from_lot(self):    
        for picking in self:
            move_line = self.env['stock.move.line'].search([('lot_id', '=', picking.lot_id.id)], limit=1)
            picking.dispatch_from_lot = move_line.dispatchs if move_line else False
